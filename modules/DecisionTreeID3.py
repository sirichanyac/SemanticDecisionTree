...
class DecisionTree:
    ...
    def build_Tree(self):
        ...
        if(len(self.dataset.columns) <= 2):  # There is only one attribute left
            ...
            best_attribute, Best_split = findBestAttribute(self.dataset, self.target) 
            #("Chosen best Attribute ", best_attribute)
            ...
            for category, data in best_attribute_dataGroup:
                ...
                subtree = DecisionTree(data, self.target,self.max_depth, category, best_attribute,self.dept)
