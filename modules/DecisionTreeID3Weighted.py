from modules.util import findMostFrequentClass, findBestAttributeWeighted
from modules.Node import Node
import math
import numpy as np

class DecisionTreeWeighted:
    def __init__(self, dataset, target, weight,max_depth, category = None, attribute = None,dept =None):
    ...
    def build_Tree(self):
        ...
        if(len(self.dataset.columns) <= 2):  # There is only one attribute left
            ...
            best_attribute, Best_split = findBestAttributeWeighted(self.dataset, self.target, self.weight) 
            ...
            for category, data in best_attribute_dataGroup:
                ...
                subtree = DecisionTreeWeighted(data, self.target, self.weight,self.max_depth,category, best_attribute,self.dept)
